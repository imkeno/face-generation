...
def discriminator(images, reuse=False, alpha=0.01):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        droupout1 = tf.layers.dropout(relu1,rate=0.45)
        # 14x14x64
        #print(relu1)
        x2 = tf.layers.conv2d(droupout1, 128, 5, strides=2, padding='same',
                             kernel_initializer=tf.contrib.layers.xavier_initializer())
        bn2 = tf.layers.batch_normalization(x2, training=True)
        relu2 = tf.maximum(alpha * bn2, bn2)
        droupout2 = tf.layers.dropout(relu2,rate=0.45)
        # 7x7x128
        #print(relu2)
        x3 = tf.layers.conv2d(droupout2, 256, 5, strides=2, padding='same',
                             kernel_initializer=tf.contrib.layers.xavier_initializer())
        bn3 = tf.layers.batch_normalization(x3, training=True)
        relu3 = tf.maximum(alpha * bn3, bn3)
        droupout3 = tf.layers.dropout(relu3,rate=0.45)
        # 4x4x256
        #print(relu3)
        # Flatten it
        flat = tf.reshape(droupout3, (-1, 4*4*256))
